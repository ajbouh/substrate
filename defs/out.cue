...
if #out.buildx_bake_metadata_raw != "" {
  ...
  #out: buildx_bake_image_ids_file: strings.Join([
    for key, buildx_bake_metadata_entry in #out.buildx_bake_metadata if (enable[key]) {
      """
...
#out: systemd_overlay_quadlets_dir: string @tag(systemd_overlay_quadlets_dir)
#out: systemd_overlay_units_dir: string @tag(systemd_overlay_units_dir)
#out: systemd_overlay_presets_dir: string @tag(systemd_overlay_presets_dir)

#out: systemd_overlay_txtar: (txtar & {
  #files: {
    for basename, unit in #out.systemd_quadlets {
      "\(#out.systemd_overlay_quadlets_dir)/\(basename)": (systemd.#render_unit & {#unit: unit}).#out
    }
    for basename, unit in #out.systemd_units {
      "\(#out.systemd_overlay_units_dir)/\(basename)": (systemd.#render_unit & {#unit: unit}).#out
    }

    let preset_header = "Autogenerated by out.cue"
    for basename, preset in #out.systemd_presets {
      "\(#out.systemd_overlay_presets_dir)/\(basename)": (systemd.#render_preset & {#preset: preset, #header: preset_header}).#out
    }
  }
}).#out
