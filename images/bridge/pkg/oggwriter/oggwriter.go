...
type OggWriter struct {
	...
	previousGranuleIncrement uint64
...
func (i *OggWriter) WriteRTP(packet *rtp.Packet, samples int) error {
	if packet == nil {
	...
	increment := int(packet.Timestamp - i.previousTimestamp)
	if i.pageIndex > 2 && uint64(increment) != i.previousGranuleIncrement {
		fmt.Printf("previous write claimed to include %d samples, but this packet incremented timestamp by %d. dropped packet(s)?\n", i.previousGranuleIncrement, increment)
		samples = increment
	}
	i.previousGranuleIncrement = uint64(samples)
	i.previousGranulePosition += uint64(samples)
