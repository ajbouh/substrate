...
type WriteTx struct {
	...
	triggerTx   *TriggerTx
	at          event.ID
	dataTxes    map[event.ID]EventDataTx
}
...
func (tx *WriteTx) finishData(commit, breakEarly bool) error {
	var errs []error
	for _, committer := range tx.dataTxes {
		err := committer.Finish(commit)
		if err != nil {
			if breakEarly {
				return err
			} else {
...
func (tx *WriteTx) shouldReactionRun(ctx context.Context, id event.ID, t *Trigger) (bool, error) {
	if t == nil {
		return false, nil
	}
	if t.Alarm != nil {
		// if the alarm is in the past, we need to run it.
		slog.Info("WriteTx.shouldReactionRun", "id", id, "t", t, "t.Alarm.Compare(event.Time(id))", t.Alarm.Compare(event.Time(id)))
		if t.Alarm.Compare(event.Time(id)) <= 0 {
			return true, nil
		}
	}
	...
	if len(t.QuerySet) > 0 {
		...
		some, err := renderTriggerQuery(ctx, tx.VectorManifoldResolver, *t)
		if err != nil {
			return false, err
		}
...
func (tx *WriteTx) considerReactionWrite(ctx context.Context, id event.ID, fields []byte) error {
	...
	defer func() {
		slog.Info("WriteTx.considerReactionWrite", "id", id, "fpk", fpk, "type", typeField, "isDeletion", isDeletion, "err", err, "isDisabled", isDisabled, "t", t, "t", t, "shouldRun", shouldRun)
	}()
	...
	if typeField != "reaction" && typeField != "reaction-continuation" {
		return nil
	}
	...
	case "reaction-continuation":
		isDisabled = gjson.GetBytes(fields, "reaction-continuation.disabled").Bool()
	}
	...
	t = reactionTriggerFor(id, fpk, fields)

	// update the triggers
	if t == nil || len(t.QuerySet) == 0 {
		if err = tx.triggerTx.Drop(ctx, fpk); err != nil {
			return err
		}
	} else {
		if err = tx.triggerTx.Drop(ctx, t.FingerprintKey); err != nil {
			return err
		}
		if err = tx.triggerTx.Create(ctx, *t); err != nil {
			return err
		}
	}

	if shouldRun, err = tx.shouldReactionRun(ctx, id, t); err != nil {
		return err
	} else if shouldRun {
		if err = tx.triggerTx.Notify(ctx, id, fpk); err != nil {
			return err
		}
	}
