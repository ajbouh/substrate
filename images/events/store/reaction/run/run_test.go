...
func (tc *testCase) Run(t *testing.T) {
	...
	if tc.in == nil && tc.source != "" {
		tc.in = reaction(tc.source)
	}
	...
	if tc.querier != nil {
		...
		querier = &reactiontest.Querier{
			QueryEventsFunc: func(ctx context.Context, q *event.Query) ([]event.Event, reactiontest.ID, bool, error) {
				...
				// if err != nil {
				// 	return nil, reactiontest.ID{}, false, err
				// }
				// idQuery := &event.Query{
				// 	BasisCriteria: event.Criteria{
				...
				// 	},
				// }
				// idQueryB, err := json.Marshal(idQuery)
				// if err != nil {
				// 	return nil, reactiontest.ID{}, false, err
				// }
				// t.Logf("QueryEventsFunc q=%s idq=%s", string(b), string(idQueryB))
				// if string(b) == string(idQueryB) {
				// }
	...
	gotPending, err := reactionrun.Run(context.Background(), querier, tc.maxID, tc.id, []byte(tc.in))
