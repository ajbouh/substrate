...
func (step *Step) MakeResult(pause *Pause) (*event.PendingEventSet, error) {
	...
	if pause.IsAborted() {
		...
		slog.Info("MakeResult", "step.reaction", step.reaction)

		if step.reaction.Type != "reaction-continuation" {
			fingerprint, err := event.FingerprintFor(step.reaction.ShadowRawFields)
			if err != nil {
				return nil, fmt.Errorf("reaction: continuation not allowed because can't fingerprint fields: %w", err)
			}
			b := []byte(`{"type":"reaction-continuation","self":["type","reaction-continuation.shadows.id"],"reaction-continuation":{"shadows":{"id":"`)
			b = append(b, []byte(step.reaction.ID.String())...)
			b = append(b, '"', ',')
			b = append(b, []byte(`"fingerprint":`)...)
			b = append(b, fingerprint...)
			b = append(b, '}', '}', '}')
			this = b
		}

		this, err = sjson.SetBytes(this, "reaction-continuation.journal", step.resume.Journal)
		errs = append(errs, err)

		when := map[string]any{}

		if len(pause.ResumptionConditions.Query) > 0 {
			when["query"] = pause.ResumptionConditions.Query
			when["after"] = pause.ResumptionConditions.After
		}
		if pause.ResumptionConditions.Alarm != nil {
			when["alarm"] = pause.ResumptionConditions.Alarm.UnixMilli()
		}

		if len(when) > 0 {
			this, err = sjson.SetBytes(this, "reaction-continuation.when", when)
			errs = append(errs, err)
		}
	} else {
		...
		// no abort, no resume means we aren't a continuation anymore. mark the type "reaction"
		// again, unless it's been set to something else
		if step.reaction.Type == "reaction-continuation" &&
			gjson.GetBytes(this, "type").String() == "reaction-continuation" {
			this, err = sjson.SetBytes(this, "type", "reaction")
			errs = append(errs, err)
		}
	...
	if shouldWriteThis {
		step.pendingWrites.Append(
			this,
			func() (io.ReadCloser, error) {
				return storeio.NewReadSeekCloserForBytes([]byte(step.reaction.Source)), nil
			}, nil, nil)
	}
