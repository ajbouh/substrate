...
func TestReactionWrite(t *testing.T) {
	...
	c.Write(O{"fields": O{
		...
		"reaction": O{
			"when":   O{"alarm": 0},
			"type":   "text/javascript",
			"source": `(function react(api) { api.write([{fields: {foo:"bar"}}]) })`,
		}}})
...
func TestReactionLog(t *testing.T) {
	...
	c.Write(O{"fields": O{
		...
		"reaction": O{
			"when":   O{"alarm": 0},
			"type":   "text/javascript",
			"source": `(function react(api) { api.log("hi"); api.log("always"); })`,
		}}})
...
func TestReactionAsyncPause(t *testing.T) {
	...
	c.Write(O{"fields": O{
		...
		"reaction": O{
			"when": O{"alarm": 0},
			"type": "text/javascript",
			"source": `(async function react(api) {
					api.log("hi");
					await api.await({alarm: 0});
					api.log("always");
					await api.await({alarm: Date.now()+1000});
					api.log("never");
					return [];
				})`,
		}}})
...
func TestReactions(t *testing.T) {
	testCases := []struct {
		...
		// 		O{"fields": O{"name": "alice"}},
		// 	},
		// 	wantLogs: O{"log": []any{O{"args": []any{"ok"}}}},
		// },
		// {
		// 	name: "simple reaction write",
		// 	write: []any{
