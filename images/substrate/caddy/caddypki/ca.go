...
func (ca *CA) Provision(ctx context.Context, id string, log *slog.Logger) error {
	ca.mu = new(sync.RWMutex)
	ca.log = log.With("name", "ca."+id)
	ca.ctx = ctx
	...
	if authorityConfig.SignWithRoot {
		...
		signerOption = authority.WithX509SignerFunc(func() ([]*x509.Certificate, crypto.Signer, error) {
			...
			ca.log.Debug("using intermediate signer",
				...
				"not_before", issuerCert.NotBefore.String(),
				"not_after", issuerCert.NotAfter.String())
...
func (ca CA) loadOrGenRoot() (rootCert *x509.Certificate, rootKey crypto.Signer, err error) {
	slog.Info("ca.loadOrGenRoot", "root", ca.Root, "storage", ca.Storage, "ca", ca)
	if ca.Root != nil {
		return ca.Root.Load()
	}
