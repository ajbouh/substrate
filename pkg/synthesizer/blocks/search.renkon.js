...
const search = Events.receiver()

const querysetInit = Events.once(queryset);
((querysetInit) => {
    Events.send(search, "i am hungry")
})(querysetInit);
...
const searchid = Events.collect(undefined, search, ({current}={}, search) => ({stale: current ? [current] : undefined, current: genchars(10)}))
