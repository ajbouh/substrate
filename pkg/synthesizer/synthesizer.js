...
export function synthesizer() {
    ...
    const panelWrite = Events.receiver()
    ...
    const surfaceBlock = bootstrapModules.blocks.surfaceBlock()
    Renkon.setupProgram([...Renkon.scripts, ...surfaceBlock.scripts]);

    
    const msgindexURL = (() => {
        const url = new URL(baseURI);
        return url.searchParams.get("msgindex");
    })();
    const bootstrapMsgindex = bootstrapModules.msg.reflect(msgindexURL);
    const msgindexUpdated = Events.select(
        undefined,
        Events.change(bootstrapMsgindex), (now, msgindex) => ({msgindex}),
        ready, (now, ready) => (now),
    );


    const key = "surface"
    const recordsUpdated = Events.observe((notify) => {
  
        const ch = new window.MessageChannel()
        const send = ch.port1
        const recv = ch.port2
        recv.onmessage = ({data: notification}) => notify(notification)
        const panelRecordsQuery = {key: [key], query: surfaceBlock.fields.query, stream: true, port: send, [Renkon.app.transferSymbol]: [send]}
        console.log({panelRecordsQuery})
        setTimeout(() => Events.send(recordsQuery, panelRecordsQuery), 10)
        return () => {
    ...
    const processRecordsQuery = (now, queries) => {
        const added = []
    ...
    const recordsQueries = Behaviors.select(
        ...
        recordsQuery, processRecordsQuery,
        close, (now, closes) => {
