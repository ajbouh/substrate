...
write_os_container_units_overlay() {
  ...
  preset_basename=50-containers.preset
  preset_dir=$OVERLAY_BASEDIR/usr/lib/systemd/system-preset/
  preset=$preset_dir/$preset_basename
...
case "$1" in
  ...
  containers-push)
    ...
    # HACK disable for now because podman build is SO SLOW
    # build_os_images
