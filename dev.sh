...
case "$1" in
  ...
  reload)
    ...
    sudo systemctl restart substrate caddy nvidia-ctk-cdi-generate
  ...
  os-fetch)
    ...
    podman build tools/nvidia-kmods/ --output type=local,dest=os/overrides/rpm

    write_os_container_units_overlay os/gen/overlay.d/substrateos usr/share/containers/systemd usr/lib/systemd/system-preset

    # These are static binaries that we want to embed directly in the OS
    podman build images/gotty/ --target=gotty --output type=local,dest=os/gen/overlay.d/substrateos/usr/bin/
    podman build images/caddy/ --target=caddy --output type=local,dest=os/gen/overlay.d/substrateos/usr/bin/
